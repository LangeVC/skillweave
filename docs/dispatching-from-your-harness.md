...
|--------------|-------------|----------------------------------------------------------------------|
| `opencode`   | DECLARED   | A real dispatch through the seam launched `/opt/homebrew/bin/opencode run --model faigate/deepseek-v4-pro -` with the work handed over stdin, and the seam recorded `harness_source: declared`. The interactive agent boots and streams (stderr shows `providerID=faigate`, title model `deepseek-v4-pro`), then waits for an interactive turn and does not self-terminate; the subprocess seam times out at 30 s with empty stdout (`termination: timed_out`). The proof script derives `proven` from a clean `exited` termination, so this run records `proven: false` even though the harness *declaration* succeeded — that conflation (declaration vs. termination) is the finding, recorded as-is in `docs/dispatch-2-report.json`, not a masked success. |
