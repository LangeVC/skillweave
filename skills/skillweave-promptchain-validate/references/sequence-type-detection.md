...
## Format first: topic sequence or build sequence?

Before deciding plan/build/mixed, decide which **container** the input is.
The two formats have different contracts and different consuming flows:

| Format | Produced by | Consumed by | Validate |
|--------|-------------|-------------|----------|
| Topic sequence (twelve sections) | human / older template | standalone prompt-chain player, copilot inline | the twelve-section contract |
| Build sequence (`execution-sequences.yaml`) | `regen-sequence.py` | `promptchain-execute` | `phases`, `parallel_lanes`, `mutual_exclusion`, `gate_pass_requires`, `session_boundary` |

### Signals the input is a build sequence
- It has a `phases:` block with `level`, `session`, `dispatches_total`, and
  lanes grouped under `parallel_lanes:` / `serialized_lanes:`.
- It carries `mutual_exclusion:`, `gate_pass_requires:`, or
  `session_boundary: batch`.
- It starts with the header ``# generated by scripts/regen-sequence.py``.

If any signal matches, validate the build contract and stop — do NOT run the
twelve-section checklist. Name the consuming flow (`promptchain-execute`) in
the report rather than guessing which format the author meant.

A build sequence may be **plan-heavy or build-heavy in content**, so after
pinning the container you still apply the plan/build/mixed heuristics below to
the *intent* of its lanes.
